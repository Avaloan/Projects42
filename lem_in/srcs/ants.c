...
/*   Updated: 2018/03/02 02:28:19 by snedir           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	s_ants(t_env *e, t_send_ants *send_ants, t_ants *tab_ants)
{
	while (send_ants->turn < send_ants->papp)
	{
		...
		while (++send_ants->i2 < send_ants->ants)
		{
			if (tab_ants[send_ants->i2].path != -1)
			{
				tab_ants[send_ants->i2].deep_level += 1;
				ROOM = g_room(tab_ants[I2].path, tab_ants[I2].deep_level, e);
				if (send_ants->i2 == send_ants->ants - 1)
					printfourmi(e, send_ants->i2 + 1, send_ants->room, 666);
				else
					printfourmi(e, send_ants->i2 + 1, send_ants->room, 665);
				if (send_ants->room == e->end)
					tab_ants[send_ants->i2].path = -1;
			}
		}
